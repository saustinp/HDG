...
p(lb,:) = [fnval(spwb(1),-2*pp(lb,1)),fnval(spwb(2),-2*pp(lb,1))];
nr(lb,:) = [fnval(spwbder(1),-2*pp(lb,1)), fnval(spwbder(2),-2*pp(lb,1))];
nrs = sqrt(nr(lb,1).^2+nr(lb,2).^2);
nr(lb,:) = [nr(lb,2)./nrs, -nr(lb,1)./nrs];
nr(lb,:) = 0.5*(-2*nr(lb,:).*pp(lb,1) + nf(1,:).*(2+2*pp(lb,1)));
nrs = sqrt(nr(lb,1).^2+nr(lb,2).^2);
nr(lb,:) = nr(lb,:)./nrs;
p(lb,:) = p(lb,:) + nr(lb,:).*pp(lb,2);
...
p(lt,:) = [fnval(spwt(1),2*(pp(lt,1)-2)),fnval(spwt(2),2*(pp(lt,1)-2))];
nr(lt,:) = [fnval(spwtder(1),2*(pp(lt,1)-2)), fnval(spwtder(2),2*(pp(lt,1)-2))];
nrs = sqrt(nr(lt,1).^2+nr(lt,2).^2);
nr(lt,:) = -[nr(lt,2)./nrs, -nr(lt,1)./nrs];

nr(lt,:) = 0.5*(2*nr(lt,:).*(pp(lt,1)-2) + 2*nf(end,:).*(4-pp(lt,1)));
nrs = sqrt(nr(lt,1).^2+nr(lt,2).^2);
nr(lt,:) = nr(lt,:)./nrs;


p(lt,:) = p(lt,:) + nr(lt,:).*pp(lt,2);

...
function [S_j, I_j, Itilde_j, nondimensionalJump, avgJump] = jumpShockSensor(mesh,master,UDG,hFlag,fieldNo,smoothSensorFlag, plotFlag)
...
for face = 1:nf
    if mesh.f(face,end) > 0     % Interior face
        ...
        avgJump_num = avgJump_num + intNum;
        avgJump_den = avgJump_den + intDen;
    else
        % Boundary face
    end
end
avgJump = avgJump_num / avgJump_den;
...
elseif hFlag == 1   % Use average h = measure^(1/nd)
    measure = abs(computeElemMeasure(mesh,master));
    hElem = measure.^(1/nd);
    hElem = hElem(:);
...
nondimensionalJump = absJump ./ (faceMeasure .* fieldAvg);
Itilde_j = absJump ./ (faceMeasure .* fieldAvg);

I_j_p1CG = p0DG_to_p1CG(I_j, mesh);
% nondimensionalJump = p0DG_to_p1CG(nondimensionalJump, mesh);

I_j = repmat(I_j(:)',[npv,1]);
nondimensionalJump = repmat(nondimensionalJump(:)',[npv,1]);

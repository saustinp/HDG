...
disp('Computing p1 mesh and UCG...');
if solType >= 0
    [mesh_p1,UCG_p1] = getP1meshAndUCG(mesh, UDG, projectionFlag);
else
    mesh_p1 = getP1meshAndUCG(mesh, UDG, projectionFlag);
end
...
elseif solType == 1
    numSolutionFields = size(UCG_p1,2) + 6;
end
defaultMaterialId = 0;
...
disp('num2sting node locations...');
toWrite = num2str([(1:nump1Nodes)', p_aux]);
disp('Writing node locations...');
writeStringMatrix(file, toWrite);
...
%     writeStringMatrix(file, toWrite);
% end
disp('num2sting element connectivities...');
...
disp('Writing element connectivities...');
writeStringMatrix(file, toWrite);

% Save nodal fields:
if solType == -1        % Save only x-component of node location. This can be used to visualize mesh with Paraview
    solutionMatrix = mesh_p1.p(:,1);
elseif solType == 0
...
elseif solType == 1
    ...
    if mesh.nd == 2
        ...
        uy   = (ruy - ry.*uv).*r1;
        vort_z = vx-uy;
        mach = sqrt((uv.^2+vv.^2)./(gam*(gam-1)*(E-0.5*(uv.^2+vv.^2))));
        mach = real(mach);
    ...
    elseif mesh.nd == 3
        ...
        error('solType == 1 only valid in 3D.');
    end
    
    solutionMatrix = [UCG_p1, uv, vv, vMag, p, vort_z, mach];
end
...
toWrite = 'pointField, None';        % None refers to the units of the variable
writeStringMatrix(file, toWrite);
disp('num2sting solution fields...');
toWrite = num2str([(1:nump1Nodes)', solutionMatrix]);
disp('Writing solution fields...');
writeStringMatrix(file, toWrite);

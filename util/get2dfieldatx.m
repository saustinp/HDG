function V = get2dfieldatx(mesh,UDG,x,box)
...
% compute the distance from each element to x
dist = zeros(ne,1);
for i = 1:ne        
...
% find all elements whose distance smaller than 0.5
dmax = 0.5;
inde = find(dist<dmax);
if nargin > 3
    inde = ind1(inde);
...
% get the field and coordinate on those elements

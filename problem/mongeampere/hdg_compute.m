function [M, Minv, MiB, MiC, MiD, MiE, F, H, G, L, S, D, E, P] = hdg_compute(master, mesh, app)
...
for i = 1:nt % loop over each element
    ...
    P(:,i) = shap*(master.gwvl.*jac);
    
    % allocate memory for the elemental matrices
    D1_K = zeros(npv,npv,nd,nd);
    E1_K = zeros(npv,nfe*npf,nd,nd);
    ...
    for j = 1:nfe % loop over each face of the element i
        ...
        for l=1:nd
            ...
            tmx(:,:,l) = shapfc*diag(dws.*xgf(:,l))*shapfc';
            C_K((l-1)*npv+I,J) = C_K((l-1)*npv+I,J) + tmn(:,:,l);   
            for m = 1:nd
              tm2 = shapfc*diag(dws.*nl(:,l).*nl(:,m))*shapfc';
              D1_K(I,I,l,m) = D1_K(I,I,l,m) + tau*tm2;   
              E1_K(I,J,l,m) = E1_K(I,J,l,m) + tau*tm2;   
            end
        end
        ...
        if ibf>0            
            ...
            if (ibc==1) % Neumman
                ...
                % form I_K
                I_K(J,I) = I_K(J,I) + tmp;  
            elseif (ibc==2) % Nonlinear neumman
                S_K(J,1) = S_K(J,1) + shapfc*(dws.*gb); 
                % form G_K
                for l=1:nd
                    G_K(J,(l-1)*npv+I) = G_K(J,(l-1)*npv+I) + tmn(:,:,l);   
                ...
                S_K(J,1) = S_K(J,1) + tau*shapfc*(dws.*gb); 
            end            
        end        
    end    
    D(:,:,i) = D_K;    
    ...
    if nd==2
        MiD(:,:,1,1,i) = -MiBt(:,:,1)*MiB(:,:,1,i) - Mi*D1_K(:,:,1,1);
        MiD(:,:,2,2,i) = -MiBt(:,:,2)*MiB(:,:,2,i) - Mi*D1_K(:,:,2,2);
        MiD(:,:,1,2,i) = -MiBt(:,:,2)*MiB(:,:,1,i) + Mi*D1_K(:,:,2,2);
        MiD(:,:,2,1,i) = -MiBt(:,:,1)*MiB(:,:,2,i) + Mi*D1_K(:,:,1,1);
        
        MiE(:,:,1,1,i) = Mi*E1_K(:,:,1,1) + MiBt(:,:,1)*MiC(:,:,1,i);      
        MiE(:,:,2,2,i) = Mi*E1_K(:,:,2,2) + MiBt(:,:,2)*MiC(:,:,2,i);      
        MiE(:,:,1,2,i) = -Mi*E1_K(:,:,2,2) + MiBt(:,:,2)*MiC(:,:,1,i);      
        MiE(:,:,2,1,i) = -Mi*E1_K(:,:,1,1) + MiBt(:,:,1)*MiC(:,:,2,i);      
    end
%     for l = 1:nd      
%       for m = 1:nd      

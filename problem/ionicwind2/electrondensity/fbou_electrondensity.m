...
mue_ref = param{19};
tau = param{end};
De = get_diffusion_e(normE, N);           % Multiplying by E_bd required to convert back to dimensional units
...
switch ib
    ...
    case 4  % Grounded surface
        ...
        fh(:,1) = r.*(De_s.*dne_dr.*nl(:,1) + De_s.*dne_dz.*nl(:,2)) + tau.*(ne-uh(:,1));
        fh(:,2) = r.*(Dn_s.*dnn_dr.*nl(:,1) + Dn_s.*dnn_dz.*nl(:,2)) + tau.*(nn-uh(:,2));

        fh_udg(:,1,1) = tau;                    % dfh(ne)_d(ne)
        fh_udg(:,1,5) = De_s.*r.*nl(:,1);     % dfh(ne)_d(dne_dr)
        fh_udg(:,1,9) = De_s.*r.*nl(:,2);     % dfh(ne)_d(dne_dz)

        fh_udg(:,2,2) = tau;                    % dfh(nn)_d(nn)
        fh_udg(:,2,6) = Dn_s.*r.*nl(:,1);     % dfh(nn)_d(dnn_dr)
        fh_udg(:,2,10) = Dn_s.*r.*nl(:,2);     % dfh(nn)_d(dnn_dz)

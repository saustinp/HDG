...
UDG = initu(mesh2,{ui(1),ui(2),ui(3),ui(4),0,0,0,0,0,0,0,0});
UH = inituhat(master,mesh2.elcon,UDG,app.ncu);
SH = [];
...
[UDG,UH] = hdg_solve(master,mesh2,app,UDG,UH,SH);

mesh2.dgnodes(:,3,:) = 0.0125*dist;
[UDG,UH] = hdg_solve(master,mesh2,app,UDG,UH,SH);
figure(2); clf; scaplot(mesh2,UDG(:,1,:),[],2,0); axis off; 
hold on; 
plot(pn(:,1),pn(:,2),'-r','LineWidth',3);

mesh2.dgnodes(:,3,:) = 0.005*dist;
[UDG,UH] = hdg_solve(master,mesh2,app,UDG,UH,SH);
figure(2); clf; scaplot(mesh2,UDG(:,1,:),[],2,0); axis off; 

UDG02 = UDG; UH02=UH;
alpha = 100; beta = 0.1; href = 0.1; hk = 0.001;
av = [1 1/2 1/4 1/8 1/16 1/32 1/64 1/128]*0.05;

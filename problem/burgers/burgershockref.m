...
% shock location and exact solution
% x0 = linspace(0,1,1e4);
% xs = 0*y;
% for i = 1:length(y)     
%     [~, um] = exactsolution(x0, y(i), alpha, beta, gamma);
%     up = 2*x0/y(i);
% %     if i>1
% %         up = 2*(x0-xs(i-1))/y(i);
% %     end
...
div = UDGprev(:,2,:) + 0*UDGprev(:,3,:);
s = ucg2udg(udg2ucg(div, cgent2dgent, rowent2elem), cgelcon);
figure(3); clf;scaplot(mesh, s , [-1 10],2,0); axis equal; axis tight;

% s0 = 10; s1 = 40; nref = 6; d = 2; n = 20; m = 1000; polydegree=6;
...
% figure(1);clf; plot(xts(:,1),xts(:,2),pn(:,1),pn(:,2),'--');
% 
% x1 = linspace(pn(1,1),pn(end,1),200);
% y1 = polyval(poly,x1);
% figure(1);clf; plot(xts(:,1),xts(:,2),x1,y1,'--');

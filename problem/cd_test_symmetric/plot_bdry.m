function [unique_pts, bdry_sol] = plot_bdry(mesh, UDG_history, bdry_ind, sol_field, timeslice)
...
for i=1:size(elems_on_bdry)
    ...
    % if no match, search for the the negative face

    locNodes = mesh.perm(:,locFaceInd);

    dgSol(1+(i-1)*nodesPerFace:i*nodesPerFace) = UDG_history(locNodes,sol_field,elem_idx,timeslice);
...
% 
% plot(unique_pts(:,2), sol_unique/(5e-4)^3);

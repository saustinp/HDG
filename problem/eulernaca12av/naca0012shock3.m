function mesh = naca0012shock3(poly, poly2, porder)
...
idx = find(abs((p(:,1)-0.299).^2 + (p(:,2)+0.06).^2)<1e-6);
p(idx,2) = -naca12(p(idx,1));
idx = find(abs((p(:,1)-0.442).^2 + (p(:,2)-0.056).^2)<1e-6);
p(idx,2) = naca12(p(idx,1));
...
pv = [0.5426 0.7110; 0.6701 0.6998; 0.966 5.9e-3; 0.977 -4.3e-3; 0.31 -0.125; 0.356 -0.123];

function mesh = mkmesh_naca12shock(poly, poly2, porder)

yu = linspace(0.04326, 0.7172, 21);
xu = polyval(poly,yu);

yl = linspace(-0.16794,-0.0597, 5);
xl = polyval(poly2,yl);

du = [0 0.0008 0.002 0.004 0.008 0.016 0.032 0.064];
du = [-du(end:-1:2) du];
theta = -pi/36; nu = [cos(theta) sin(theta)];
[pu,tu] = shockgrid(xu, yu, du, nu, 0);

dl = [0 0.001 0.003 0.0075 0.015 0.027];
dl = [-dl(end:-1:2) dl];

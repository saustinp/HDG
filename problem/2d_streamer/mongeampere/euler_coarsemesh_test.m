...
iter = 1;
% Ma_array = [3, 3.5, 4, 5, 6, 7, 9, 11];
Ma_array = [5]
for Mach = Ma_array(iter:end)
    disp("========Running Mach number=====" + string(Mach));
    str = "mach_sweep_data_coarse/Mach"+string(Mach);
    nstage = 1;
    torder = 1;
    % Mach   = 5;
    aoa    = 0.0;
...
%     
    ...
    mesh = mkmesh_halfcircle(meshsq,1,5,10,pi/2,3*pi/2);
...
% 
%      disp("=================")
%     disp("Manual AV Iter 5")
%     disp("=================")
%     mesh.dgnodes(:,3,:) = 0.01*tanh(meshdist(mesh,2)*5);
%     [UDG,UH] = hdg_solve(master,mesh,app,UDG,UH,SH);
%     
    ...
    % Initial solve
    disp("=================")
    disp("Running AV loop 1")
    disp("=================")
    [UDG1, UH1, ACG1, mine1, minf1, ming1, mesh1] = hdgsolve_avloop2(master, mesh, app, mesh.dgnodes(:,1:2,:), UDG, UH, 0.04, 4, 1);
    figure(1); clf; scaplot(mesh, eulereval(UDG1{end},'M',gam,Minf),[],2); 
    
    % Solve Monge Ampere
    disp("=================")
    disp("Running MA iteration 1")
    disp("=================")
    [u1,q1,uhat1,v1,rho1,iter1] = hdgsolve_mah(master, mesh, [], UDG1{end}, 20e-3, []);
    ...
    % Solve on new mesh
    disp("=================")
    disp("Running AV loop 2")
    disp("=================")
    [UDG2, UH2, ACG2, mine2, minf2, ming2, mesh2] = hdgsolve_avloop2(master, mesh, app, q1, UDG0, UH0, 0.02, 5,1);

    %%%% Taken from euler1
    disp("=================")
    disp("Running MA iteration 2")
    disp("=================")
    [u2,q2,uhat2,v2,rho2,iter2] = hdgsolve_mah(master, mesh, mesh2, UDG2{end}, 30e-3, [], 0.25);
    [elist, xi] = locatexinmesh(mesh2, q2, [], 1e-4);
    UDG0 = evalfield(mesh2, UDG2{end}, elist, xi);
    UDG0 = permute(reshape(UDG0, [master.npe mesh.ne 12]),[1 3 2]); 
    UH0 = inituhat(master,mesh.elcon,UDG0,app.ncu);

    disp("=================")
    disp("Running AV loop 3")
    disp("=================")
        [UDG3, UH3, ACG3, mine3, minf3, ming3, mesh3] = hdgsolve_avloop2(master, mesh, app, q2, UDG0, UH0, 0.02, 5,5);

%     figure(3); clf; scaplot(mesh3, eulereval(UDG3{5},'M',1.4,7),[],1,2); axis on; axis equal; axis tight;
    meshstruct_refine{iter} = mesh3;
    UDG3struct{iter} = UDG3;
    ...
    save(str+"_res");
%     save(str+"_res.mat","UDG1","UDG2","UDG3","mesh1","mesh2","mesh3","");
...
%     meshtmpstruct = {meshstruct{1}, meshstruct{end}};
    ...
    for i = 1:8
        Minf = Ma_array(i);
        UDGtmp_struct = UDG3struct_fix{i};
        meshtmp = meshstruct_refine_fix{i};
...
%          UDG2struct{1}
            ...
            ylabel("\rho")
            set(gca,'FontSize',18); 
            leg_arr = [leg_arr, "$M_{\infty} = "+string(Ma_array(i))+"$"];
%             leg = legend(["$M_{\infty} = 2$", "$M_{\infty} = 3$", "$M_{\infty} = 4$", "$M_{\infty} = 5$", "$M_{\infty} = 7$", "$M_{\infty} = 9$", "$M_{\infty} = 11$"], 'interpreter', 'latex', 'FontSize', 16, 'Location', 'NW');
    ...
    leg = legend(leg_arr,'interpreter', 'latex', 'FontSize', 16, 'Location', 'NW');
...
%         set(gca,'FontSize',18);
        ...
        figure(4); meshplot(mesh,1)
%         title("Mach " + string(Ma_array(i)))
%         set(gca,'FontSize',18);
%         title('$\mu=$'+string(Minf),'Interpreter','latex')
title("Reference mesh",'Interpreter','latex')
        set(gca,'FontSize',18); 
        axis on; box on; axis tight;
...
%     plot(x1{2},t1{2}/Tinf_array(2),'-.', 'Color', [0.6350 0.0780 0.1840],'LineWidth',2,'MarkerSize',8);
    ...
    Ma_array_train = Ma_array(1:2:end);
    p_rb = length(Ma_array_train)

    phi_Ma={};
    i_train = 1:2:length(Ma_array);
    for i = 1:p_rb
    ...
    for i = 1:p_rb
        ...
        UDG3tmp_smooth = UDG3tmp{1}(:,1,:);
        UDG3tmp_sharp = UDG3tmp{4}(:,1,:);
...
%         MPhi_UDG2_sharp(:,i) = apply_weight_to_vector(M, Phi_UDG2_sharp(:,i),mesh,master);
        ...
        Phi_UDG3_smooth(:,i) = UDG3tmp_smooth(:);
%         MPhi_UDG3_smooth(:,i) = apply_weight_to_vector(M, Phi_UDG3_smooth(:,i),mesh,master);
        Phi_UDG3_sharp(:,i) = UDG3tmp_sharp(:);
%         MPhi_UD32_sharp(:,i) = apply_weight_to_vector(M, Phi_UDG3_sharp(:,i),mesh,master);
...
for i = 1:length(Ma_array_test)
    str = "mach_sweep_data_coarse/Mach"+string(Ma_array_test(i));
...
for i = 1:length(Ma_array_test)
    ...
    wkspc_tmp = load(str+"_res");
%     mesh_tmp = mesh;
    UDG1_tmp = wkspc_tmp.UDG3{4}(:,1,:);
    mesh_tmp = wkspc_tmp.mesh3;
%     figure(1); scaplot(mesh, UDG1_tmp);
%     tst = Wmodes1*(pinv(Wmodes1)*UDG1tmp(:));
    alpha = pinv(Wmodes3_sharp)*UDG1_tmp(:);
%     disp(alpha);
    tst = Wmodes3_sharp*(alpha);

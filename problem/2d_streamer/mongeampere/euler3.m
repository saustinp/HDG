...
% iter_curr = 1;
Ma_array = [9];
for Mach = Ma_array(iter_curr:end)
    disp("========Running Mach number=====" + string(Mach));
    str = "mach_sweep_data_refine/Mach"+string(Mach);
    load("mach_sweep_data/Mach"+string(Mach)+"_res");
%     s = sensor(mesh2, master, UDG2{4}, 2e-2);

    %%%% Taken from euler1
    [u2,q2,uhat2,v2,rho2,iter2] = hdgsolve_mah(master, mesh, mesh2, UDG2{1}, 30e-3, [], 0.25);
...
%     figure(3); clf; scaplot(mesh3, eulereval(UDG3{5},'M',1.4,7),[],1,2); axis on; axis equal; axis tight;
    meshstruct_refine{iter_curr} = mesh3;
    UDG3struct{iter_curr} = UDG3;
    iter_curr = iter_curr + 1;

    

%%%%% MY CODE
    % [u2,q2,uhat2,v2,rho2,iter2] = hdgsolve_mah(master, mesh, mesh2, UDG2{4}, 2e-2,[]);
    % qt2 = ucg2udg(udg2ucg(q2, cgent2dgent, rowent2elem), cgelcon);
    % mesht = mesh; mesht.dgnodes = qt2;
    
    % [elist, xi] = locatexinmesh(mesh2, qt2, [], 1e-6);
    % UDG0 = evalfield(mesh2, UDG2{4}, elist, xi);
    % UDG0 = permute(reshape(UDG0, [master.npe mesh.ne 12]),[1 3 2]); 
    % UH0 = inituhat(master,mesh.elcon,UDG0,app.ncu);
    % figure(1); clf; scaplot(mesh2, eulereval(UDG2{4},'M',1.4,Mach),[],2); colormap jet;
    % figure(2); clf; scaplot(mesht, eulereval(UDG0,'M',1.4,Mach),[],2); colormap jet;
    
    % % [UDG0,Ua,Ub] = initshock(mesht,kk);
    % % UH0 = inituhat(master,mesht.elcon,UDG0,app.ncu);
    % % [UDGt, UHt, ACGt] = hdgsolve_avloop(master, mesh, app, qt2, UDG0, UH0, 0.035, 5);
    
    % [UDG3, UH3, ACG3, mine3, minf3, ming3, mesh3] = hdgsolve_avloop(master, mesh, app, qt2, UDG0, UH0, 0.01, 4);

    % [elist_tmp, xi_tmp] = locatexinmesh(mesh3, mesh.dgnodes(:,1:2,:), [], 1e-6);
    % UDG_tst = evalfield(mesh3, UDG3{1}, elist_tmp, xi_tmp);
%%%%%%
...
% 
%     disp("=================")
%     disp("Manual AV Iter 1")
%     disp("=================")
%     mesh.dgnodes(:,3,:) = 0.09*mesh.dist;
...
%     
%     disp("=================")
%     disp("Manual AV Iter 2")
%     disp("=================")
%     mesh.dgnodes(:,3,:) = 0.04*mesh.dist;
%     [UDG,UH] = hdg_solve(master,mesh,app,UDG,UH,SH);
%     
%     disp("=================")
%     disp("Manual AV Iter 3")
%     disp("=================")
%     mesh.dgnodes(:,3,:) = 0.03*mesh.dist;
%     [UDG,UH] = hdg_solve(master,mesh,app,UDG,UH,SH);
%     
%     disp("=================")
%     disp("Manual AV Iter 4")
%     disp("=================")
...
%     meshtmpstruct = {meshstruct{1}, meshstruct{end}};
    ...
    for i = 1:2
        Minf = Ma_array(i);
        UDGtmp_struct = UDG3struct{i};
        meshtmp = meshstruct_refine{i};
...
%         figure(i); clf; 
        ...
        meshtmp = meshstruct_refine{i};
        UDGtmp = UDGtmp_struct{1};
        figure(1); subplot(1,3,cnt); scaplot(mesh, eulereval(UDGtmp,'M',1.4,Minf), [0 Minf])
        title('$\mu=$'+string(Minf),'Interpreter','latex')
        set(gca,'FontSize',18); 
        axis on; box on; axis tight;

        figure(2); subplot(1,3,cnt); scaplot(meshtmp, eulereval(UDGtmp,'M',1.4,Minf), [0 Minf])
...
%     plot(x1{2},t1{2}/Tinf_array(2),'-.', 'Color', [0.6350 0.0780 0.1840],'LineWidth',2,'MarkerSize',8);
    ...
    Phi_UDG2_smooth = zeros(npe*ncu*ne,p_rb);
    Phi_UDG2_sharp = zeros(npe*ncu*ne,p_rb);
    for i = 1:p_rb
        q = phi_Ma{i};
        UDG1tmp = UDG1struct_refine{i};
        UDG1tmp = UDG1tmp{end}(:,1:ncu,:);
        UDG2tmp = UDG2struct_refine{i};
        UDG2tmp_smooth = UDG2tmp{1}(:,1:ncu,:);
        UDG2tmp_sharp = UDG2tmp{end}(:,1:ncu,:);
...
[V_x,D_x] = eig(Phi_x'*Phi_x); %%TODO: should multiply by mass matrix to have correct inner product
...
[V_y,D_y] = eig(Phi_y'*Phi_y);
[~,jj] = sort(diag(D_y),'descend');
D_y = D_y(jj,jj);
V_y = V_y(:,jj);
Wmodes_y = Phi_y*V_y;
svals_y = diag(D_y);
figure(1); clf; semilogy(svals_x/svals_x(1), 'LineWidth',2)
figure(2); clf; semilogy(svals_y/svals_y(1), 'LineWidth',2)


[~,svals_mapped_smooth] = snapshot_POD(Phi_UDG2_smooth);
[Wmodes2,svals_mapped_sharp] = snapshot_POD(Phi_UDG2_sharp);
...
for i = 1:7
    ...
    figure(4);
    subplot(1,2,1); scaplot(mesh, eulereval(tst1,'p',1.4,Minf),[-1 1],2);
    subplot(1,2,2); scaplot(mesh, eulereval(tst2,'p',1.4,Minf),[-1 1],2);

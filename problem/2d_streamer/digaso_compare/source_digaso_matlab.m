function [s,s_udg] = source_digaso_matlab(s, s_udg, pg, udg, param, time, ng, nc, ncu, nd, ncd)
    ...
    coder.ceval('source', coder.ref(s), coder.ref(s_udg), coder.ref(pg), coder.ref(udg), coder.ref(param), int32(time), int32(ng), int32(nc), int32(ncu), int32(nd), int32(ncd)); 
    
    end

    % Make the fname a variable so it can extend to other cases
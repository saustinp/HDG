...
%         pres = (gam-1)*(uh(:,4) - 0.5*(uh(:,2).*u1 + uh(:,3).*u2) + uh(:,1).*k);
%         pinf = (uinf(:,4)-0.5)*(gam-1);
%         fh(:,4) = pinf - pres./uh(:,1);
% 
%         r = uh(:,1);
...
%         fh_udg = cat(3,fh_u,reshape(fh_q,ng,nch,2*nch));      
    case 16  % Adiabatic Wall     
        fh(:,1) = 1.0017-uh(:,1);
        fh(:,2) = -1.0017*p(:,2)-uh(:,2);
        fh(:,3) = 1.0017*p(:,1)-uh(:,3);
        fh(:,4) = 7.5248-uh(:,4);
        fh_uh = zeros(ng,nch,nch);        
        fh_uh(:,1,1) = -1;        
        fh_uh(:,2,2) = -1;        
...
%         fh_uh(:,3,3) = -1./(uh(:,1));
%         
%         [fh4,fh4_udg,fh4_uh] = fhat_ns2d(nl,p,udg,uh,param,time);
%         fh(:,4) = fh4(:,1);
%         fh_u(:,4,:) = fh4_udg(:,1,1:nch);
%         fh_q(:,4,:) = fh4_udg(:,1,nch+1:3*nch);                
%         fh_uh(:,4,:) = fh4_uh(:,1,:);
%         
% %         udg(:,2:3) = 0;
% %         uh(:,2:3) = 0;

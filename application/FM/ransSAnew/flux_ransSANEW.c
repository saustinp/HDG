...
void flux_ransSANEW(double *f, double *f_udg, double *pg, double *udg, double *udg_ref, double *avField_p1CG, meshstruct &mesh, masterstruct &master, appstruct &app, double *param,
                 double time, int ng, int nc, int ncu, int nd, int ncd, Int* ndims, int computeJacobian)
{
    ...
    if (app.AVflag != 0)
        AVflux_ransSA(f, f_udg, pg, udg, udg_ref, app, param, time, ng, nc, ncu, nd, ncd, computeJacobian);

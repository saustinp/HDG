...
elseif (nd==2) && (axissym==0) % NS in polar coordinates                                                  
    ...
    s = s./xr;        
%     s = [0, -rv*vv-p-tyy, rv*uv+txy, 0];
%     s = s+f(1:4);
%     s = -s./xr;    
    
%     txx = Re1*c23*(2*ux - vy); % r r
%     txy = Re1*(uy + vx);  % r theta    
%     tyy = Re1*c23*(2*vy - ux); % theta theta
%         
%     fv = [0, txx, txy, uv*txx + vv*txy + fc*Tx, ...
%           0, txy, tyy, uv*txy + vv*tyy + fc*Ty];
% 
...
%     tyy = Re1*c23*(2*(vy-uv)/xr - ux); % theta theta
    ...
    %fi = [ru, ru*uv+p, rv*uv,   rw*uv,   ru*h
    s = -[ru, ru*uv-rv*vv+txx-tyy, 2*rv*uv+2*txy, rw*uv+txz, ru*h + uv*txx + vv*txy + wv*txz + fc*Tx];
    s = s./xr;                
%     fv = [0, txx, txy, uv*txx + vv*txy + fc*Tx, ...
%           0, txy, tyy, uv*txy + vv*tyy + fc*Ty/xr];
% 

...
if nc == 12
    [f,f_udg] = flux_polarns2d(p,udg,param,time);
elseif nc == 20
    [f,f_udg] = flux_ns3d(p,udg,param,time);
end
...
cst = 1;
txx = Re1*c23*(2*ux - vy - cst*u./xr);
%txy = Re1*(uy + vx);
txy = Re1*(uy./xr - cst*v./xr + vx);
tyy = Re1*c23*(2*vy - ux - cst*u./xr);
...
% ux  = (rux - rx.*u).*r1 =  (rux - rx.*ru/r).*r1

txx_r  =  Re1*c23*((4*ru.*rx-2*rv.*ry)-r.*(2*rux-rvy)).*r1.^3 - cst*Re1*c23*u_r./xr;
txx_ru = -Re1*c23*2*rx.*r1.^2 - cst*Re1*c23*u_ru./xr;
...
%txy = Re1*(uy + vx);
%txy = Re1*(uy./xr - cst*v./xr + vx);
%vx  = (rvx - rx.*v).*r1 = (rvx - rx.*rv.*r1).*r1;
%uy  = (ruy - ry.*u).*r1 = (ruy - ry.*ru.*r1).*r1;
txy_r  =  Re1*(2*(ru.*ry./xr+rv.*rx)-r.*(ruy./xr+rvx)).*r1.^3 - cst*Re1*v_r./xr;
txy_ru =  -Re1*ry.*r1.^2./xr;
txy_rv =  -Re1*rx.*r1.^2 - Re1*cst*r1./xr;
txy_rE =  zero;
...
txy_ry  = -Re1*ru.*r1.^2./xr;
txy_ruy =  Re1*r1./xr;
txy_rvy =  zero;
txy_rEy =  zero;

% tyy = Re1*c23*(2*vy - ux - cst*u./xr);
tyy_r  =  Re1*c23*((4*rv.*ry-2*ru.*rx)-r.*(2*rvy-rux)).*r1.^3 - cst*Re1*c23*u_r./xr;
tyy_ru =  Re1*c23*rx.*r1.^2 - cst*Re1*c23*u_ru./xr;

...
void fbou(double *fh, double *fh_u, double *fh_uh, double *pg, double *udg, double *udg_ref, double *uhg, double *uhg_ref,
          double *nl, double *avtg_p1CG, double *ui, meshstruct &mesh, masterstruct &master, appstruct &app,
          double *param, double time, Int ib, Int* ndims, int computeJacobian, int numPoints)
{
...
// // // //                         ui[3] = sqrt(ui[1]*ui[1] + ui[2]*ui[2]) + 1/(gam*gam1*M2_infty);
        ...
        fhatDriver(fh, fh_u, fh_uh, pg, udg, uhg, udg_ref, uhg_ref, nl, avtg_p1CG, mesh, master, app, param, time, ndims, fhatExpression, computeJacobian, numPoints);       // TODO: Impose energy BC in another way, since this won't work for ALE
...
//                     
//                     ui_g[0] = udg[0*ng+i];//(udg[(1+nd)*ng+i] - kinEnrgy) / T_w;//udg[0*ng+i];//
//                     for (n = 0; n < nd; n++)
//                         ui_g[1+n] = 0.0;
//                     ui_g[1+nd] = udg[0*ng+i] * T_w;//(udg[(1+nd)*ng+i] - kinEnrgy);//udg[0*ng+i] * T_w;//
//                     
//                     nm = k * ng * (nd + 2) + j * ng + i;
//                     
// //                     if (j==k) {
...
//         delete[] an; delete[] An; delete[] anm; delete[] Anm; delete[] ui_g;
        ...
        // IMPLEMENTATION NO. 2:
//         double kinEnrgyInf = 0.0, T_infty;
//         for (j=0; j<nd; j++)
//             kinEnrgyInf += 0.5*ui[1+j]*ui[1+j]/ui[0];
//         T_infty = (ui[1+nd] - kinEnrgyInf) / ui[0];
//         
//         double ratio = 500.0/200.0;     // 500.0/200.0: Value for cylinder at M = 18, Re = 378k in Barer's thesis
        ...
        for (i=0; i<ng; i++) {
            ...
            fh[(1+nd)*ng+i] = T_w - (uhg[(1+nd)*ng+i] - kinEnrgy) / uhg[0*ng+i];       // This line replaces the following line from the MATLAB implementation
...
//                fh_uh[(1+nd)*sz2 + (1+nd)*ng + i] = - gam * gam1 * M2_infty / uhg[0 * ng + i];
        ...
        for (j=0; j<nch; j++)
            for (i=0; i<ng; i++)
                uinf[i+j*ng] = ui[j];   // This loop replaces the next loop from the MATLAB implementation
//        for (i=0; i<ng*nch; i++)
//            uinf[i] = udg[i];
        for (i=0; i<ng; i++) {
        ...
        if (computeJacobian == 1) {
            for (k=0; k<nch; k++)
                for (j=0; j<nch; j++)
                    for (i=0; i<ng; i++) {
                        nm = k*sz2+j*ng+i;
                        uinfu[nm] = 0.0;        // This line replaces the next line from the MATLAB implementation
...
//     }
        ...
        double *uPlus = new double[ng * nc];
        double *uMinus = new double[ng * nc];
        ...
        // Boundary conditions for mass, momentum and energy equations:
        Int fhatExpression = 0;
        fhatDriver(fh, fh_u, fh_uh, pg, udg, uhg, udg_ref, uhg_ref, nl, avtg_p1CG, mesh, master, app, param, time, ndims, fhatExpression, computeJacobian, numPoints);       // TODO: This won't work for ALE
        fhatExpression = 2;
        fhatDriver(fPlus, fPlus_u, fPlus_uh, pg, uPlus, uhg, udg_ref, uhg_ref, nl, avtg_p1CG, mesh, master, app, param, time, ndims, fhatExpression, computeJacobian, numPoints);       // TODO: This won't work for ALE
        fhatDriver(fMinus, fMinus_u, fMinus_uh, pg, uMinus, uhg, udg_ref, uhg_ref, nl, avtg_p1CG, mesh, master, app, param, time, ndims, fhatExpression, computeJacobian, numPoints);       // TODO: This won't work for ALE
        for (j = 0; j < (2 + nd); j++)
            for (i = 0; i < ng; i++)
        ...
        if (computeJacobian == 1) {
            ...
            for (k = ncu; k < nc; k++)
                for (j = 0; j < (2 + nd); j++)
                    for (i = 0; i < ng; i++) {
                        nm2 = k * sz2 + j * ng + i;
                        fh_u[nm2] -= 0.5 * (fPlus_u[nm2] + fMinus_u[nm2]);
                    }
        }

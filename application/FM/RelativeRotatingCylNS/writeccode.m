...
%     OmegaCrossR = cross(Omega,R);
    ...
    txx = Re1*c23*(2*ux - (vy - uv)/x); % r r
    txy = Re1*((uy+vv)/x + vx);  % r theta    
    tyy = Re1*c23*(2*(vy-uv)/x - ux); % theta theta
    
    fv = [0, txx, txy, uv*txx + vv*txy + fc*Tx, ...
          0, txy, tyy, uv*txy + vv*tyy + fc*Ty/x];
    f = fi+fv;
    f(5:8) = f(5:8)./x;
    
    s = -[ru, ru*uv-rv*vv+txx-tyy, 2*rv*uv+2*txy, ru*h + uv*txx + vv*txy + fc*Tx];
    
    sOmega = -[0, -r*(omega*x)^2-2*omega*x*rv, 2*omega*x*ru, 0];
    s = s+sOmega;   
    s = s./x;          
    
%     Momentum = [udg(2:nd+1) 0];
%     OmegaCrossMomentum = cross(Omega,Momentum);
%     OmegaCrossOmegaCrossR = cross(Omega,OmegaCrossR);
%     s = -[0 2*OmegaCrossMomentum + r*OmegaCrossOmegaCrossR];
    
    nx = nl(1);
    ny = nl(2);
    fh  = fc*(Tx*nx+Ty*ny/(x*x)) + tau*(udg(4)-uh(4));    
else                                         
    r    = udg(1);
    ru   = udg(2); % r
    rv   = udg(3); % theta
...
%     tyy = Re1*c23*(2*(vy-uv)/xr - ux); % theta theta
    ...
    s = -[ru, ru*uv-rv*vv+txx-tyy, ru*vv+rv*uv+2*txy, rw*uv+txz, ru*h + uv*txx + vv*txy + wv*txz + fc*Tx];            
    sOmega = -[0, -r*(omega*xr)^2-2*omega*xr*rv, 2*omega*xr*ru, 0, 0];
    s = s+sOmega;   
    s = s./xr;          
...
%     OmegaCrossMomentum = cross(Omega,Momentum);
%     OmegaCrossOmegaCrossR = cross(Omega,OmegaCrossR);
%     s = -[0 2*OmegaCrossMomentum+r*OmegaCrossOmegaCrossR 0];    
end

...
% % Anm     = zeros(ng,nc,nc,nc);        
    ...
    case 2  % Adiabatic Wall     
        ...
        [fbn,fbn_udg,fbn_uh] = fhat_ns3d(nl,pg,udg,uh,param,time);
        ...
        fb_u(:,1,1) = ones(ng,1);        
        fb_u(:,nch,:) = fbn_udg(:,1,1:nch);
   
        fb_q = zeros(ng,nch,nch*nd);
        fb_q(:,nch,:) = fbn_udg(:,1,nch+1:(nd+1)*nch);
    ...
    case 15 % Isothermal wall
        ...
        xr = pg(:,1);        
        k = 0.5*((omega*xr).^2);        
        pres = (gam-1)*(uh(:,5) - 0.5*(uh(:,2).*u1 + uh(:,3).*u2 + uh(:,4).*u3) + uh(:,1).*k);
        pinf = (uinf(:,5)-0.5)*(gam-1);
        fb(:,5) = pinf - pres./uh(:,1);
        ...
        fb_uh(:,2,1) = uh(:,2)./(uh(:,1).^2);
        fb_uh(:,2,2) = -1./(uh(:,1));
        fb_uh(:,3,1) = uh(:,3)./(uh(:,1).^2);
        fb_uh(:,3,3) = -1./(uh(:,1));
        fb_uh(:,4,1) = uh(:,4)./(uh(:,1).^2);
        fb_uh(:,4,4) = -1./(uh(:,1));        
        ...
        rw = uh(:,4);
        rE = uh(:,5);
        %fb_uh(:,5,1) = -((gam - 1)*(ru.^2./(2*r) + rv.^2./(2*r) - rE))./r.^2 - ((ru.^2./(2*r.^2) + rv.^2./(2*r.^2))*(gam - 1))./r;        
        %fb_uh(:,4,1) = -(gam-1)*0.5*(uh(:,2).^2+uh(:,3).^2)./uh(:,1).^2; %gam*gam1*M2*[-uh(:,4)./uh(:,1).^2, zero, zero, 1./uh(:,1)];
        fb_uh(:,5,1) = -((gam - 1)*(ru.^2 + rv.^2 + rw.^2 - r.*rE))./(r.^3);
        fb_uh(:,5,2) = (gam-1)*u1./r;
        fb_uh(:,5,3) = (gam-1)*u2./r;
        fb_uh(:,5,4) = (gam-1)*u3./r;
        fb_uh(:,5,5) = (1-gam)./r;
    ...
    case 16 % Isothermal wall
        ...
        [fb5,fb5_udg,fb5_uh] = fhat_ns3d(nl,pg,udg,uh,param,time);
        fb(:,5) = fb5(:,1);
        fb_u(:,5,:) = fb5_udg(:,1,1:nch);
        fb_q(:,5,:) = fb5_udg(:,1,nch+1:4*nch);                
        fb_uh(:,5,:) = fb5_uh(:,1,:);

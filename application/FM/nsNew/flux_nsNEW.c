...
void flux_nsNEW(double *f, double *f_udg, double *pg, double *udg, double *udg_ref, double *avField, meshstruct &mesh, masterstruct &master, appstruct &app, double *param,
             double time, int ng, int nc, int ncu, int nd, int ncd, Int* ndims, int computeJacobian)
{
    ...
    if (app.AVflag != 0)
        AVflux_ns(f, f_udg, pg, udg, udg_ref, avField, mesh, master, app, param, time, ng, nc, ncu, nd, ncd, ndims, computeJacobian);

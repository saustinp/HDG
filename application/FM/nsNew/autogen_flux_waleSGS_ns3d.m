...
% - Deviatoric SGS viscous stress tensor model: WALE. Ref: http://www.scholarpedia.org/article/Turbulence:_Subgrid-Scale_Modeling
...
syms pg time
syms Axx Axy Axz Ayx Ayy Ayz Azx Azy Azz
syms Sxx Sxy Sxz Syx Syy Syz Szx Szy Szz S_mag S_fr2
syms Oxx Oxy Oxz Oyx Oyy Oyz Ozx Ozy Ozz
syms Wxx Wxy Wxz Wyx Wyy Wyz Wzx Wzy Wzz
syms Wdxx Wdxy Wdxz Wdyx Wdyy Wdyz Wdzx Wdzy Wdzz Wd_fr2
syms h C_w C_i Pr_SGS Delta nu_SGS
syms x y z h
...
% Resolved velocity Jacobian matrix:
Axx = ux;
Axy = uy;
Axz = uz;
Ayx = vx;
...
S_mag = sqrt( 2.0 * (Sxx*Sxx + Sxy*Sxy + Sxz*Sxz + ...
                     Syx*Syx + Syy*Syy + Syz*Syz + ...
                     Szx*Szx + Szy*Szy + Szz*Szz) );

% Resolved rotation rate tensor:
Oxx = 0;
Oxy = 0.5 * (- vx + uy);
Oxz = 0.5 * (- wx + uz);
Oyx = 0.5 * (- uy + vx);
...
Wxx = Sxx*Sxx + Sxy*Syx + Sxz*Szx + Oxx*Oxx + Oxy*Oyx + Oxz*Ozx;
Wxy = Sxx*Sxy + Sxy*Syy + Sxz*Szy + Oxx*Oxy + Oxy*Oyy + Oxz*Ozy;
Wxz = Sxx*Sxz + Sxy*Syz + Sxz*Szz + Oxx*Oxz + Oxy*Oyz + Oxz*Ozz;
Wyx = Syx*Sxx + Syy*Syx + Syz*Szx + Oyx*Oxx + Oyy*Oyx + Oyz*Ozx;
Wyy = Syx*Sxy + Syy*Syy + Syz*Szy + Oyx*Oxy + Oyy*Oyy + Oyz*Ozy;
Wyz = Syx*Sxz + Syy*Syz + Syz*Szz + Oyx*Oxz + Oyy*Oyz + Oyz*Ozz;
Wzx = Szx*Sxx + Szy*Syx + Szz*Szx + Ozx*Oxx + Ozy*Oyx + Ozz*Ozx;
Wzy = Szx*Sxy + Szy*Syy + Szz*Szy + Ozx*Oxy + Ozy*Oyy + Ozz*Ozy;
Wzz = Szx*Sxz + Szy*Syz + Szz*Szz + Ozx*Oxz + Ozy*Oyz + Ozz*Ozz;

% Deviatoric part of W:
Wdxx = Wxx - (Wxx + Wyy + Wzz) / 3;
Wdxy = Wxy;
Wdxz = Wxz;
Wdyx = Wyx;
Wdyy = Wyy - (Wxx + Wyy + Wzz) / 3;
Wdyz = Wyz;
Wdzx = Wzx;
Wdzy = Wzy;
Wdzz = Wzz - (Wxx + Wyy + Wzz) / 3;

% Frobenius norm of S:
S_fr2 = Sxx*Sxx + Sxy*Sxy + Sxz*Sxz + ...
        Syx*Syx + Syy*Syy + Syz*Syz + ...
        Szx*Szx + Szy*Szy + Szz*Szz;

% Frobenius norm of Wd:
Wd_fr2 = Wdxx*Wdxx + Wdxy*Wdxy + Wdxz*Wdxz + ...
         Wdyx*Wdyx + Wdyy*Wdyy + Wdyz*Wdyz + ...
         Wdzx*Wdzx + Wdzy*Wdzy + Wdzz*Wdzz;

% Eddy kinematic viscosity:
nu_SGS = (C_w * Delta)^2 * Wd_fr2^(3/2) / (S_fr2^(5/2) + Wd_fr2^(5/4));
...
fprintf(gid, '%s\n\n',str);
str = ['void ' fileName '(double *f, double *f_udg, double *pg, double *udg, appstruct &app, double *param, double time, int ng, int nc, int ncu, int nd, int ncd, int computeJacobian)'];
fprintf(gid, '%s\n', str); 
str = '{';
fprintf(gid, '%s\n', str);         

...
% - Deviatoric SGS viscous stress tensor model: Vreman. Ref: http://www.scholarpedia.org/article/Turbulence:_Subgrid-Scale_Modeling
...
syms pg time
syms Axx Axy Axz Ayx Ayy Ayz Azx Azy Azz A_fr2
syms Sxx Sxy Sxz Syx Syy Syz Szx Szy Szz S_mag S_fr2
syms Gxx Gxy Gxz Gyx Gyy Gyz Gzx Gzy Gzz Q_G
syms h C_v C_i Pr_SGS Delta nu_SGS
syms x y z h
...
% Values for C_v:
% 1) A. W. Vreman, An eddy-viscosity subgrid-scale model for turbulent shear
% flow: Algebraic theory and applications: C_v = 2.5*C_smag^2 \approx 0.07225.
% They use 0.07 for numerical experiments.
% 2) CHARLES: 0.07
...
% Tensor G:
Gxx = Delta^2 * (Axx*Axx + Axy*Axy + Axz*Axz);
Gxy = Delta^2 * (Axx*Ayx + Axy*Ayy + Axz*Ayz);
Gxz = Delta^2 * (Axx*Azx + Axy*Azy + Axz*Azz);
Gyx = Delta^2 * (Ayx*Axx + Ayy*Axy + Ayz*Axz);
Gyy = Delta^2 * (Ayx*Ayx + Ayy*Ayy + Ayz*Ayz);
Gyz = Delta^2 * (Ayx*Azx + Ayy*Azy + Ayz*Azz);
Gzx = Delta^2 * (Azx*Axx + Azy*Axy + Azz*Axz);
Gzy = Delta^2 * (Azx*Ayx + Azy*Ayy + Azz*Ayz);
Gzz = Delta^2 * (Azx*Azx + Azy*Azy + Azz*Azz);

% Second invariant of G:
Q_G = 0.5 * ((Gxx + Gyy + Gzz)^2 - ...
             (Gxx*Gxx + Gxy*Gyx + Gxz*Gzx + ...
              Gyx*Gxy + Gyy*Gyy + Gyz*Gzy + ...
              Gzx*Gxz + Gzy*Gyz + Gzz*Gzz));

% Frobenius norm of A:
A_fr2 = Axx*Axx + Axy*Axy + Axz*Axz + ...
        Ayx*Ayx + Ayy*Ayy + Ayz*Ayz + ...
        Azx*Azx + Azy*Azy + Azz*Azz;

% Eddy kinematic viscosity:
nu_SGS = C_v * sqrt(Q_G / A_fr2);

...
txx_AV = r.*av.*(ux + ux - (2.0/3.0)*(ux+vy));
txy_AV = r.*av.*(uy + vx);
tyx_AV = r.*av.*(vx + uy);
tyy_AV = r.*av.*(vy + vy - (2.0/3.0)*(ux+vy));

% Artificial viscosity fluxes:
F_AV = [0; txx_AV; txy_AV; txx_AV.*u + txy_AV.*v; 0; tyx_AV; tyy_AV; tyx_AV.*u + tyy_AV.*v];

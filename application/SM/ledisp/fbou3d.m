...
switch (ib)
    case 1
        ...
        fb = param3*reshape(fb,ng,nch);
        fb_udg = param3*reshape(fb_udg,ng,nch,nc);
        fb_uh = param3*reshape(fb_uh,ng,nch,nch);
    case 2
        nl1 = nl(:,1);
        nl2 = nl(:,2);
        nl3 = nl(:,3);
    ...
    case 8
        ...
        fb_udg = reshape(fb_udg,ng,nch,nc);
        fb_uh = reshape(fb_uh,ng,nch,nch);
    case 9 % Sinusoidal Neumann Boundary conditions
        nl1 = nl(:,1);
        nl2 = nl(:,2);
        nl3 = nl(:,3);
        one = ones(ng,1);
        ...
        t5 = u6+u10;
        t6 = u9+u11;
        fb = [uinf1*sin(2*pi*freq*time)+param3.*(u1-uh1)+nl1.*(t3+param1.*u4.*2.0)+nl2.*param1.*t4+nl3.*param1.*t5;uinf2*sin(2*pi*freq*time)+param3.*(u2-uh2)+nl2.*(t3+param1.*u8.*2.0)+nl1.*param1.*t4+nl3.*param1.*t6;uinf3*sin(2*pi*freq*time)+param3.*(u3-uh3)+nl3.*(t3+param1.*u12.*2.0)+nl1.*param1.*t5+nl2.*param1.*t6];
        if nargout > 1
            t7 = one.*param3;
            t8 = nl1.*one.*param1;
            t9 = nl2.*one.*param1;
        ...
        fb_udg = reshape(fb_udg,ng,nch,nc);
        fb_uh = reshape(fb_uh,ng,nch,nch);
    case 10 % Sinusoidal Dirichlet Boundary conditions
        one = ones(ng,1);
        uh1 = uh(:,1);
        uh2 = uh(:,2);
        uh3 = uh(:,3);
        ...
        freq = param{5};
        zero = zeros(ng,1);
        fb = [-one.*(uh1-uinf1*sin(2*pi*freq*time));-uh2+uinf2*sin(2*pi*freq*time);-uh3+uinf3*sin(2*pi*freq*time)];
        if nargout > 1
            fb_udg = [zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero;zero];
        end
        if nargout > 2

function [fb,fb_udg,fb_uh] = fboudyn2d(ib,uinf,nl,pg,udg,uh,param,time)
%FBOU2D.M
...
switch (ib)
    ...
    case 3
        ...
        uinf2 = uinf(:,2);
        zero = zeros(ng,1);
        fb = [one.*(-uh1+uinf1);uinf2+tau.*(u2-uh2)+nl2.*(lambda.*(q11+q22+2.0)+mu.*(q22+1.0).*2.0)+mu.*nl1.*(q12+q21)];
        if nargout > 1
            t2 = mu.*nl1.*one;
            t3 = one.*tau;
    ...
    case 4
        ...
        uinf2 = uinf(:,2);
        zero = zeros(ng,1);
        fb = [uinf1+tau.*(u1-uh1)+nl1.*(lambda.*(q11+q22+2.0)+mu.*(q11+1.0).*2.0)+mu.*nl2.*(q12+q21);-uh2+uinf2];
        if nargout > 1
            t2 = mu.*nl2.*one;
            t3 = one.*tau;

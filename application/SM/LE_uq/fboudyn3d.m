function [fb,fb_udg,fb_uh] = fboudyn3d(ib,uinf,nl,pg,udg,uh,param,time)
%FBOU3D.M
...
switch (ib)
    ...
    case 3
        ...
        t3 = lambda.*t2;
        t4 = q23+q32;
        fb = [one.*(-uh1+uinf1);uinf2+nl2.*(t3+mu.*(q22+1.0).*2.0)+tau.*(u2-uh2)+mu.*nl1.*(q12+q21)+mu.*nl3.*t4;uinf3+nl3.*(t3+mu.*(q33+1.0).*2.0)+tau.*(u3-uh3)+mu.*nl1.*(q13+q31)+mu.*nl2.*t4];
        if nargout > 1
            t5 = one.*tau;
            t6 = mu.*nl1.*one;
            t7 = lambda.*nl3.*one;
    ...
    case 4
        ...
        t3 = lambda.*t2;
        t4 = q13+q31;
        fb = [uinf1+nl1.*(t3+mu.*(q11+1.0).*2.0)+tau.*(u1-uh1)+mu.*nl2.*(q12+q21)+mu.*nl3.*t4;-uh2+uinf2;uinf3+nl3.*(t3+mu.*(q33+1.0).*2.0)+tau.*(u3-uh3)+mu.*nl2.*(q23+q32)+mu.*nl1.*t4];
        if nargout > 1
            t5 = one.*tau;
            t6 = mu.*nl2.*one;
            t7 = lambda.*nl3.*one;
    ...
    case 5
        ...
        t3 = lambda.*t2;
        t4 = q12+q21;
        fb = [uinf1+nl1.*(t3+mu.*(q11+1.0).*2.0)+tau.*(u1-uh1)+mu.*nl3.*(q13+q31)+mu.*nl2.*t4;uinf2+nl2.*(t3+mu.*(q22+1.0).*2.0)+tau.*(u2-uh2)+mu.*nl3.*(q23+q32)+mu.*nl1.*t4;-uh3+uinf3];
        if nargout > 1
            t5 = one.*tau;
            t6 = mu.*nl2.*one;
            t7 = mu.*nl1.*one;
    ...
    case 6
        ...
        uinf3 = uinf(:,3);
        zero = zeros(ng,1);
        fb = [one.*(-uh1+uinf1);-uh2+uinf2;uinf3+nl3.*(lambda.*(q11+q22+q33+3.0)+mu.*(q33+1.0).*2.0)+tau.*(u3-uh3)+mu.*nl1.*(q13+q31)+mu.*nl2.*(q23+q32)];
        if nargout > 1
            t2 = lambda.*nl3.*one;
            t3 = mu.*nl1.*one;
            t4 = mu.*nl2.*one;
    ...
    case 7
        ...
        uinf3 = uinf(:,3);
        zero = zeros(ng,1);
        fb = [uinf1+nl1.*(lambda.*(q11+q22+q33+3.0)+mu.*(q11+1.0).*2.0)+tau.*(u1-uh1)+mu.*nl2.*(q12+q21)+mu.*nl3.*(q13+q31);-uh2+uinf2;-uh3+uinf3];
        if nargout > 1
            t2 = mu.*nl2.*one;
            t3 = mu.*nl3.*one;
            t4 = lambda.*nl1.*one;
    ...
    case 8
        ...
        uinf3 = uinf(:,3);
        zero = zeros(ng,1);
        fb = [one.*(-uh1+uinf1);uinf2+nl2.*(lambda.*(q11+q22+q33+3.0)+mu.*(q22+1.0).*2.0)+tau.*(u2-uh2)+mu.*nl1.*(q12+q21)+mu.*nl3.*(q23+q32);-uh3+uinf3];
        if nargout > 1
            t2 = mu.*nl1.*one;
            t3 = mu.*nl3.*one;
            t4 = lambda.*nl2.*one;
    ...
    case 9
        ...
        t5 = q13+q31;
        t6 = q23+q32;
        fb = [uinf1*sin(2*pi*freq*time)+nl1.*(t3+mu.*(q11+1.0).*2.0)+tau.*(u1-uh1)+mu.*nl2.*t4+mu.*nl3.*t5;uinf2*sin(2*pi*freq*time)+nl2.*(t3+mu.*(q22+1.0).*2.0)+tau.*(u2-uh2)+mu.*nl1.*t4+mu.*nl3.*t6;uinf3*sin(2*pi*freq*time)+nl3.*(t3+mu.*(q33+1.0).*2.0)+tau.*(u3-uh3)+mu.*nl1.*t5+mu.*nl2.*t6];
        if nargout > 1
            t7 = one.*tau;
            t8 = mu.*nl1.*one;
            t9 = mu.*nl2.*one;

...
void fluxDriver(double * f,double * f_UDG, double * pg, double * UDG, double * UDG_ref, double *avField, meshstruct &mesh, masterstruct &master, appstruct &app, double * param, double time,
                Int* ndims, int computeJacobian, int numPoints)
{
    ...
    if (ALEflag == 2 || ALEflag == 3) {
        ...
        UDG2udg(udg_ref, UDG_ref, pg, app, numPoints, ncu, ncq, nc, nd);
        if (computeJacobian == 1)
    ...
    switch (app.appname) {
        case 0:
            flux_eulerNEW(f, f_udg, pg, udg, udg_ref, avField, mesh, master, app, param, time, numPoints, nc, ncu, nd, ncd, ndims, computeJacobian);
            break;
        case 1:
            flux_nsNEW(f, f_udg, pg, udg, udg_ref, avField, mesh, master, app, param, time, numPoints, nc, ncu, nd, ncd, ndims, computeJacobian);
            break;
        case 3:
            flux_ransSANEW(f, f_udg, pg, udg, udg_ref, avField, mesh, master, app, param, time, numPoints, nc, ncu, nd, ncd, ndims, computeJacobian);
            break;
        default: {

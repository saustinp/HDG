function Ru = cuda_Rface(Ru, udg, xdg, uinf, tempn, tempg, shapfn, shapfg, param, time, f2e, nn, nmf)
%% This function is meant to mimic the functionality of Exasim's RuFace
%      The functions cuda_Ru_face forms the face residual and assembles it
%      into the global residual block-by-block. We instead want the entire
%      RuFace array formed for the nodal-based stamping
...
for j=1:nbf(1)
   ...
   % Need to insert this output into the face residual
   Ru(:,:,f1:f2) = reshape(tempn(1:n1,1:ncu), [npf ncu ns]);

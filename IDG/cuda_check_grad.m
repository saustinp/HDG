function cuda_check(mesh, master, app, SH, UDG, UH, uinf, param, time, fc_q, fc_u, tdep, f2e, nn, nme, nmf)
    ...
    %TODO: mesh argument is temporary
    %TODO: This Cuda_Ru_face function forms the face residuals and immediately assembles them into the global residual
    %      In order to test the nodal-based stamping, we need to form and store the face residual first. 
    Ru2 = Ru;
    Ru = Ru;
    Rf = zeros(npf,ncu,nf);
    shapfgdotshapfc = master.shapfgdotshapfc;
%     Ru = cuda_Ru_face(Ru, UDG, mesh.dgnodes, uinf, tempfn, tempfg, shapfn, shapfg, shapfgdotshapfc, param, time, f2e, nn, nmf,mesh);
    [Ru, Rftst,Ip,Im] = cuda_Ru_face(Ru, UDG, mesh.dgnodes, uinf, tempfn, tempfg, shapfn, shapfg, param, time, f2e, nn, nmf);
    Im = sort(Im,1);
    Ip = sort(Ip,1);
    Rf = cuda_Rface(Rf, UDG, mesh.dgnodes, uinf, tempfn, tempfg, shapfn, shapfg, param, time, f2e, nn, nmf);
    [Ru2,IpN,ImN] = cuda_putfacenodes_nodal_based(Ru2, Rf, mesh.rowe2f1, mesh.cole2f1, mesh.ent2ind1, mesh.rowe2f2, mesh.cole2f2, mesh.ent2ind2, npf, npe, nc, 0, ne, 0);
    ImN = sort(ImN,1);
    IpN = sort(IpN,1);
    app.fbou = 'ldgfbou';
    app.fhat = 'ldgfhat';

function [udgx] = dgfieldatx(mesh,UDG,idx,x,nref)
...
udg = permute(UDG(:,:,idx),[1 3 2]);
pdg = permute(mesh.dgnodes(:,1:nd,idx),[1 3 2]);
...
npe = size(pdg,1);
pdg = reshape(pdg,[npe*nex nd]);
udg = reshape(udg,[npe*nex nc]);
indp = (1:npe)';
